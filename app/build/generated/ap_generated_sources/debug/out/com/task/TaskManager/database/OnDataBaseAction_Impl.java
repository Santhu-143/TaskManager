package com.task.TaskManager.database;
...
public final class OnDataBaseAction_Impl implements OnDataBaseAction {
  ...
  public OnDataBaseAction_Impl(RoomDatabase __db) {
    ...
    this.__preparedStmtOfUpdateAnExistingRow = new SharedSQLiteStatement(__db) {
      @Override
      public String createQuery() {
        final String _query = "UPDATE Task SET taskTitle = ?, taskDescription = ?, date = ?, lastAlarm = ?, event = ? WHERE taskId = ?";
        return _query;
      }
    };
  }
  ...
  public void updateAnExistingRow(final int taskId, final String taskTitle,
      final String taskDescription, final String taskDate, final String taskTime,
    ...
    if (taskTime == null) {
      ...
      _stmt.bindString(_argIndex, taskTime);
    }
    _argIndex = 5;
    if (taskEvent == null) {
      _stmt.bindNull(_argIndex);
    } else {
      _stmt.bindString(_argIndex, taskEvent);
    }
    _argIndex = 6;
    _stmt.bindLong(_argIndex, taskId);
    __db.beginTransaction();
